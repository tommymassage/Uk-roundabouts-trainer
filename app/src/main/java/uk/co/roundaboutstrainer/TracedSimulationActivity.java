...
public class TracedSimulationActivity extends Activity {
    ...
    static final class TracedOverlay extends View {
        ...
        private void buildPath(){updateImageRect();float[][] p=UserTracedRouteLibrary.pointsForExit(exit);path.reset();if(p.length<2||imageRect.isEmpty())return;path.moveTo(mapX(p[0][0]),mapY(p[0][1]));for(int i=1;i<p.length-1;i++){float x=mapX(p[i][0]),y=mapY(p[i][1]);float mx=mapX((p[i][0]+p[i+1][0])*.5f),my=mapY((p[i][1]+p[i+1][1])*.5f);path.quadTo(x,y,mx,my);}path.lineTo(mapX(p[p.length-1][0]),mapY(p[p.length-1][1]));}
