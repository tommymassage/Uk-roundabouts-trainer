...
public class TracedSimulationActivity extends Activity {
    ...
    static final class TracedOverlay extends View {
        ...
        private void startTraffic(){
            ...
            trafficAnimator.setDuration(24000); // 50% slower NPC traffic
        ...
        void reset(){if(animator!=null)animator.cancel();progress=0f;waitingAtGiveWay=false;passedGiveWay=false;collision=false;invalidate();}
        void start(){
            if(!UserTracedRouteLibrary.hasUserTrace(exit))return;
            if(collision){reset();return;}
            if(waitingAtGiveWay){waitingAtGiveWay=false;passedGiveWay=true;animateFrom(progress,1f);return;}
            if(animator!=null&&animator.isRunning())return;
            passedGiveWay=false;collision=false;animateFrom(progress,1f);
        }

        private void animateFrom(float from,float to){
            if(animator!=null)animator.cancel();
            animator=ValueAnimator.ofFloat(from,to);
            animator.setDuration(Math.max(1300L,(long)(durationForExit(exit)*(to-from))));
            animator.setInterpolator(new AccelerateDecelerateInterpolator());
            animator.addUpdateListener(a->{
                float next=(float)a.getAnimatedValue();
                if(!passedGiveWay && next>=YIELD_PROGRESS){progress=YIELD_PROGRESS;waitingAtGiveWay=true;animator.cancel();invalidate();return;}
                progress=next;
                if(passedGiveWay && progress>YIELD_PROGRESS+.015f && collidesWithTraffic(progress)){collision=true;animator.cancel();}
                invalidate();
            });
            animator.start();
        }

        private long durationForExit(int e){return e==1?9600:e==2?12400:e==3?15200:18000;}
        ...
        private void buildSpline(Path target,float[][] p){
            target.reset();if(p.length<2||imageRect.isEmpty())return;
            ...
            for(int i=0;i<p.length-1;i++){
                ...
                float c1x=x1+(mapX(p2[0])-mapX(p0[0]))*(SPLINE_TENSION/6f),c1y=y1+(mapY(p2[1])-mapY(p0[1]))*(SPLINE_TENSION/6f);
                float c2x=x2-(mapX(p3[0])-mapX(p1[0]))*(SPLINE_TENSION/6f),c2y=y2-(mapY(p3[1])-mapY(p1[1]))*(SPLINE_TENSION/6f);
        ...
        private void buildCompanionPath(){
            float[][] p=UserTracedRouteLibrary.pointsForExit(companionExit());
            // Keep the inside car clearly separated around the 12 o'clock section.
            if(companionExit()==3){
                for(int i=0;i<p.length;i++){
                    float y=p[i][1];
                    if(y<.42f){
                        float strength=1f-Math.min(1f,Math.abs(y-.20f)/.22f);
                        p[i][0]+=.030f*Math.max(.25f,strength);
                    }
                }
            }
            buildSpline(companionPath,p);
        }
        ...
        private boolean trafficConflictAtEntry(){
            ...
            float ul=pm.getLength(),nl=npcPm.getLength();if(ul<=0||nl<=0)return false;pm.getPosTan(ul*YIELD_PROGRESS,userCheck,null);
            float s=Math.min(imageRect.width(),imageRect.height()),limit=s*.115f;
            for(int i=0;i<NPC_OFFSETS.length;i++){npcPm.getPosTan(nl*npcPhase(i),npcPos,null);if((float)Math.hypot(npcPos[0]-userCheck[0],npcPos[1]-userCheck[1])<limit)return true;}return false;
        }
        private boolean collidesWithTraffic(float userProgress){
            buildPath();buildNpcPath();pm.setPath(path,false);npcPm.setPath(npcPath,false);
            float ul=pm.getLength(),nl=npcPm.getLength();if(ul<=0||nl<=0)return false;pm.getPosTan(ul*userProgress,userCheck,null);
            float s=Math.min(imageRect.width(),imageRect.height()),limit=s*.050f;
            for(int i=0;i<NPC_OFFSETS.length;i++){npcPm.getPosTan(nl*npcPhase(i),npcPos,null);if((float)Math.hypot(npcPos[0]-userCheck[0],npcPos[1]-userCheck[1])<limit)return true;}return false;
        }

        @Override protected void onDraw(Canvas c){
            super.onDraw(c);if(getWidth()==0||getHeight()==0)return;buildPath();buildCompanionPath();buildNpcPath();float s=Math.min(imageRect.width(),imageRect.height());
            if(showRoute){shadowPaint.setStrokeWidth(s*.016f);routePaint.setStrokeWidth(s*.007f);c.drawPath(path,shadowPaint);c.drawPath(path,routePaint);if(sideBySide){companionRoutePaint.setStrokeWidth(s*.006f);c.drawPath(companionPath,companionRoutePaint);}}
        ...
        private void drawTraffic(Canvas c,float s){
            ...
            for(int i=0;i<NPC_OFFSETS.length;i++){
                npcPm.getPosTan(len*npcPhase(i),npcPos,npcTan);float angle=(float)Math.toDegrees(Math.atan2(npcTan[1],npcTan[0]))+90f;
                c.save();c.translate(npcPos[0],npcPos[1]);c.rotate(angle);
                float cw=s*.0416f,ch=cw*1.72f;
                c.drawRoundRect(new RectF(-cw*.61f,-ch*.30f,-cw*.45f,-ch*.03f),cw*.06f,cw*.06f,npcTyrePaint);c.drawRoundRect(new RectF(cw*.45f,-ch*.30f,cw*.61f,-ch*.03f),cw*.06f,cw*.06f,npcTyrePaint);
                c.drawRoundRect(new RectF(-cw/2,-ch/2,cw/2,ch/2),cw*.18f,cw*.18f,npcPaint);c.drawRoundRect(new RectF(-cw*.32f,-ch*.18f,cw*.32f,ch*.08f),cw*.07f,cw*.07f,npcGlassPaint);
                c.drawCircle(-cw*.30f,ch*.42f,cw*.055f,brakePaint);c.drawCircle(cw*.30f,ch*.42f,cw*.055f,brakePaint);c.restore();
            }
        }

        private void drawStatus(Canvas c,float s){
            if(!waitingAtGiveWay&&!collision&&!sideBySide)return;String text;int col;
            if(collision){text="COLLISION - TAP START TO RESET";col=Color.argb(230,150,20,20);}else if(waitingAtGiveWay){boolean conflict=trafficConflictAtEntry();text=conflict?"GIVE WAY - TRAFFIC COMING":"CLEAR - TAP START TO ENTER";col=conflict?Color.argb(225,150,85,10):Color.argb(225,20,115,55);}else{text="2-CAR MODE: YOUR EXIT "+exit+" / BLUE CAR EXIT "+companionExit();col=Color.argb(225,20,85,150);}
            float cx=mapX(.50f),cy=mapY(.055f),w=s*.31f,h=s*.038f;statusPaint.setColor(col);c.drawRoundRect(new RectF(cx-w/2,cy-h/2,cx+w/2,cy+h/2),h*.45f,h*.45f,statusPaint);statusTextPaint.setTextSize(s*.017f);c.drawText(text,cx,cy+s*.006f,statusTextPaint);
        }
        ...
        @Override public boolean onTouchEvent(MotionEvent e){
            if(e.getAction()!=MotionEvent.ACTION_DOWN)return true;updateImageRect();if(imageRect.isEmpty()||!imageRect.contains(e.getX(),e.getY()))return true;float x=normX(e.getX()),y=normY(e.getY());
            if(x>=.010f&&x<=.185f){if(y>=.098f&&y<.132f){selectExit(1);return true;}if(y<.166f&&y>=.132f){selectExit(2);return true;}if(y<.199f&&y>=.166f){selectExit(3);return true;}if(y<=.235f&&y>=.199f){selectExit(4);return true;}}
            ...
            if(x>=.730f&&x<=.930f&&y>=.120f&&y<=.210f){start();return true;}return true;
        }
        ...
        private void drawCompanionCar(Canvas c,float s){
            companionPm.setPath(companionPath,false);float len=companionPm.getLength();if(len<=0)return;float cp=Math.max(0f,Math.min(1f,progress));companionPm.getPosTan(len*cp,companionPos,companionTan);float angle=(float)Math.toDegrees(Math.atan2(companionTan[1],companionTan[0]))+90f;drawSimpleCar(c,s,companionPos[0],companionPos[1],angle,companionPaint,companionDarkPaint,companionGlassPaint,companionExit(),cp,false);
        }
        ...
        private void drawSimpleCar(Canvas c,float s,float x,float y,float angle,Paint bodyPaint,Paint darkPaint,Paint windowPaint,int carExit,float p,boolean user){
            c.save();c.translate(x,y);c.rotate(angle);float cw=s*.0416f,ch=cw*1.72f;
            c.drawRoundRect(new RectF(-cw*.61f,-ch*.30f,-cw*.45f,-ch*.03f),cw*.06f,cw*.06f,tyrePaint);c.drawRoundRect(new RectF(cw*.45f,-ch*.30f,cw*.61f,-ch*.03f),cw*.06f,cw*.06f,tyrePaint);c.drawRoundRect(new RectF(-cw*.61f,ch*.08f,-cw*.45f,ch*.34f),cw*.06f,cw*.06f,tyrePaint);c.drawRoundRect(new RectF(cw*.45f,ch*.08f,cw*.61f,ch*.34f),cw*.06f,cw*.06f,tyrePaint);
            ...
            c.drawRoundRect(new RectF(-cw*.42f,ch*.37f,cw*.42f,ch*.49f),cw*.08f,cw*.08f,darkPaint);c.drawRoundRect(new RectF(-cw*.34f,-ch*.24f,cw*.34f,ch*.22f),cw*.14f,cw*.14f,darkPaint);c.drawRoundRect(new RectF(-cw*.28f,-ch*.18f,cw*.28f,ch*.16f),cw*.08f,cw*.08f,windowPaint);
            ...
            boolean blink=((int)(p*34))%2==0;if(blink&&leftIndicatorOn(carExit,p))c.drawCircle(-cw*.43f,-ch*.39f,cw*.08f,indicatorPaint);if(blink&&rightIndicatorOn(carExit,p))c.drawCircle(cw*.43f,-ch*.39f,cw*.08f,indicatorPaint);
            if(user&&waitingAtGiveWay){c.drawCircle(-cw*.28f,ch*.43f,cw*.08f,brakePaint);c.drawCircle(cw*.28f,ch*.43f,cw*.08f,brakePaint);}if(user&&collision){collisionPaint.setStrokeWidth(s*.006f);c.drawCircle(0,0,cw*.85f,collisionPaint);}c.restore();
        }

        private void drawCar(Canvas c,float s){pm.setPath(path,false);float len=pm.getLength();if(len<=0)return;pm.getPosTan(len*Math.max(0f,Math.min(1f,progress)),pos,tan);float angle=(float)Math.toDegrees(Math.atan2(tan[1],tan[0]))+90f;drawSimpleCar(c,s,pos[0],pos[1],angle,carPaint,carDarkPaint,glassPaint,exit,progress,true);}
    }
}
