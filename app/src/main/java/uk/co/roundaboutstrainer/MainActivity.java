...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        panel.addView(text("UK ROUNDABOUTS TRAINER  v0.6.3",24,true));
        TextView intro=text("Import a real roundabout screenshot • calibrate • train on top",13,false); intro.setPadding(0,4,0,10); panel.addView(intro);
        ...
        Button importPhoto=button("📷  IMPORT ROUNDABOUT SCREENSHOT");
        panel.addView(importPhoto,new LinearLayout.LayoutParams(-1,52));
        photoStatus=text("Built-in aerial image active",12,true); photoStatus.setPadding(0,4,0,8); panel.addView(photoStatus);

        LinearLayout calibrateRow=new LinearLayout(this); calibrateRow.setOrientation(LinearLayout.HORIZONTAL);
        Button setCentre=button("SET CENTRE"); Button rotate=button("ROTATE 90°");
        calibrateRow.addView(setCentre,new LinearLayout.LayoutParams(0,48,1)); calibrateRow.addView(rotate,new LinearLayout.LayoutParams(0,48,1)); panel.addView(calibrateRow);
        panel.addView(text("Roundabout overlay size",12,true));
        SeekBar size=new SeekBar(this); size.setMax(100); size.setProgress(50); panel.addView(size);
        ...
        TextView hint=text("For your Knightswood screenshot: import it, tap SET CENTRE, then tap the middle of the roundabout. Adjust overlay size until the green route follows the carriageway.",11,false); hint.setPadding(0,12,0,0); panel.addView(hint);
        ...
        importPhoto.setOnClickListener(v->pickImage());
        setCentre.setOnClickListener(v->{roadView.calibratingCentre=true;Toast.makeText(this,"Tap the centre of the roundabout on the image",Toast.LENGTH_LONG).show();});
        rotate.setOnClickListener(v->{roadView.photoRotation=(roadView.photoRotation+90)%360;roadView.invalidate();});
        size.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener(){public void onProgressChanged(SeekBar s,int v,boolean f){roadView.radiusScale=.72f+v/100f*.62f;roadView.invalidate();}public void onStartTrackingTouch(SeekBar s){}public void onStopTrackingTouch(SeekBar s){}});

        setContentView(root);highlight(0);
    }

    private void pickImage(){Intent i=new Intent(Intent.ACTION_OPEN_DOCUMENT);i.addCategory(Intent.CATEGORY_OPENABLE);i.setType("image/*");startActivityForResult(i,PICK_IMAGE);}
    ...
    @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){
        ...
        if(requestCode==PICK_IMAGE&&resultCode==RESULT_OK&&data!=null&&data.getData()!=null){
            ...
            try{
                ...
                InputStream in=getContentResolver().openInputStream(uri);Bitmap b=BitmapFactory.decodeStream(in);if(in!=null)in.close();
                if(b!=null){roadView.setImportedBitmap(b);photoStatus.setText("Imported screenshot active — calibrate centre + size");Toast.makeText(this,"Screenshot loaded",Toast.LENGTH_SHORT).show();}
            }catch(Exception e){Toast.makeText(this,"Could not load this image",Toast.LENGTH_LONG).show();}
        }
    }
    ...
    static class PhotoRoundaboutView extends View{
        ...
        Bitmap aerial,imported;
        boolean photoMode=true,showRoute=true,showTraffic=true,showGuide=true,calibratingCentre=false;
        int approach=0,exit=1,traffic=0,trainingMode=0,photoRotation=0;
        float raw=0,carProgress=0,centreX=.49f,centreY=.49f,radiusScale=1.03f;
        ...
        PhotoRoundaboutView(Context c){super(c);routePaint.setColor(Color.rgb(25,235,77));routePaint.setStyle(Paint.Style.STROKE);routePaint.setStrokeWidth(12);routePaint.setStrokeCap(Paint.Cap.ROUND);routePaint.setStrokeJoin(Paint.Join.ROUND);shadow.setColor(Color.argb(120,0,0,0));shadow.setStyle(Paint.Style.STROKE);shadow.setStrokeWidth(22);shadow.setStrokeCap(Paint.Cap.ROUND);loadPhoto();}
        void setImportedBitmap(Bitmap b){imported=b;photoMode=true;photoRotation=0;centreX=.50f;centreY=.52f;radiusScale=1.03f;invalidate();}
        void loadPhoto(){new Thread(()->{try{HttpURLConnection con=(HttpURLConnection)new URL(PHOTO).openConnection();con.setInstanceFollowRedirects(true);con.setConnectTimeout(12000);con.setReadTimeout(15000);con.setRequestProperty("User-Agent","UK-Roundabouts-Trainer/0.6.3");InputStream in=con.getInputStream();Bitmap b=BitmapFactory.decodeStream(in);in.close();con.disconnect();if(b!=null){aerial=b;post(this::invalidate);}}catch(Exception ignored){}}).start();}
        void reset(){if(animator!=null)animator.cancel();raw=0;carProgress=0;invalidate();}
        void start(){if(animator!=null)animator.cancel();animator=ValueAnimator.ofFloat(0,1);animator.setDuration(trainingMode==2?6500:8200);animator.setInterpolator(new LinearInterpolator());animator.addUpdateListener(a->{raw=(float)a.getAnimatedValue();float stop=traffic==0?.26f:traffic==1?.39f:.34f;if(raw<.20f)carProgress=raw/.20f*.14f;else if(raw<stop)carProgress=.14f;else carProgress=.14f+(raw-stop)/(1f-stop)*.86f;report();invalidate();});animator.start();}
        ...
        @Override public boolean onTouchEvent(android.view.MotionEvent e){if(e.getAction()==MotionEvent.ACTION_DOWN&&calibratingCentre){centreX=e.getX()/getWidth();centreY=e.getY()/getHeight();calibratingCentre=false;invalidate();return true;}return true;}

        @Override protected void onDraw(Canvas c){super.onDraw(c);float w=getWidth(),h=getHeight();Bitmap bg=imported!=null?imported:aerial;if(photoMode&&bg!=null)drawPhoto(c,bg,w,h);else drawFallback(c,w,h);float cx=w*centreX,cy=h*centreY,r=Math.min(w,h)*.255f*radiusScale;Path route=route(cx,cy,r,w,h,exit);c.save();c.rotate(approach*90,cx,cy);if(showRoute){c.drawPath(route,shadow);c.drawPath(route,routePaint);drawRouteArrows(c,route);}if(showTraffic)drawTraffic(c,cx,cy,r);drawCarOnPath(c,route,carProgress,Color.rgb(35,105,215));c.restore();if(showGuide)drawGuide(c,w,h);if(calibratingCentre){p.setColor(Color.argb(160,0,0,0));c.drawRect(0,0,w,h,p);p.setColor(Color.WHITE);p.setTextSize(Math.max(22,h*.035f));p.setTypeface(Typeface.DEFAULT_BOLD);c.drawText("TAP THE CENTRE OF THE ROUNDABOUT",w*.18f,h*.10f,p);}}

        void drawPhoto(Canvas c,Bitmap b,float w,float h){c.drawColor(Color.BLACK);c.save();c.rotate(photoRotation,w/2,h/2);float sw=b.getWidth(),sh=b.getHeight();if(photoRotation==90||photoRotation==270){float t=sw;sw=sh;sh=t;}float scale=Math.max(w/sw,h/sh);float dw=b.getWidth()*scale,dh=b.getHeight()*scale;RectF dst=new RectF((w-dw)/2,(h-dh)/2,(w+dw)/2,(h+dh)/2);c.drawBitmap(b,null,dst,p);c.restore();p.setColor(Color.argb(20,0,0,0));c.drawRect(0,0,w,h,p);}
        ...
        Path route(float cx,float cy,float r,float w,float h,int e){Path q=new Path();float lane=Math.min(w,h)*.04f;float sx=cx-lane*.7f;q.moveTo(sx,h+10);q.lineTo(sx,cy+r*1.25f);q.quadTo(sx,cy+r*1.04f,cx-r*.05f,cy+r*1.04f);float rr=r*1.04f;q.arcTo(new RectF(cx-rr,cy-rr,cx+rr,cy+rr),92,e*90);if(e==1){q.quadTo(cx-rr,cy+lane*.15f,cx-rr*1.25f,cy+lane*.15f);q.lineTo(-10,cy+lane*.15f);}else if(e==2){q.quadTo(cx-lane*.15f,cy-rr,cx-lane*.15f,cy-rr*1.25f);q.lineTo(cx-lane*.15f,-10);}else{q.quadTo(cx+rr,cy-lane*.15f,cx+rr*1.25f,cy-lane*.15f);q.lineTo(w+10,cy-lane*.15f);}return q;}
